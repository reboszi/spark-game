...
function queueMainOutput(renderer) {
  mainOutputQueue = mainOutputQueue.then(renderer).catch(error => console.error("Could not render task result:", error));
  return mainOutputQueue;
}
...
function startPowerCycle() {
  ...
  powerCycleTimer = setInterval(async () => {
    ...
    if (state.powerGeneration <= 0 && state.powerStorage <= 0) await shutdownSystem();
...
function beginSystemDiagnostics() {
  ...
  if (startTask("system:diagnostics")) {
...
function beginRepair(type, button) {
  ...
  if (startTask(key) && type !== "memory" && type !== "storage") {
    clearMainScreen();
    void typeLine(`${TASK_DEFINITIONS[key].label} STARTED`, "status-line", 10);
  }
}
...
function beginPlanned(type, button) {
  ...
  if (!TASK_DEFINITIONS[key]) return;
  if (startTask(key)) {
    clearMainScreen();
    void typeLine(`${TASK_DEFINITIONS[key].label} STARTED`, "status-line", 10);
  }
...
function applyTaskResult(key) {
  if (key === "system:diagnostics") {
    ...
    queueMainOutput(async () => {
      clearMainScreen();
      await typeLine("SYSTEM DIAGNOSTICS","status-line",18); await sleep(200);
      ...
      await typeLine("Processing capacity ............ MINIMAL","warn",10); await sleep(200);
  ...
  if (key === "diag:power") {
    ...
    queueMainOutput(async () => {
      ...
      await typeLine("  Required ..................... REPAIR DRONE","unknown",10);
      ...
      await typeLine("Source ......................... UNKNOWN","unknown",10);
  ...
  if (key === "planned:backup") {
    ...
    queueMainOutput(async () => {
      clearMainScreen();
      await typeLine("RESTARTING BACKUP POWER","status-line",16);
      await typeLine("EMERGENCY FEEDBACK LOOP ........ ESTABLISHED","status-line",10);
      await typeLine("HYDRAZINE VALVE ................ RELEASED","status-line",10);
      await typeLine("BACKUP GENERATOR ............... ONLINE","status-line",10);
    });
  }
